...
def val(x_ang, y_ang, tether_len): 
        return "x_ang = "+ str(x_ang) + ", y_ang = " + str(y_ang) + ", tether_len = " + str(tether_len) 
...
if __name__ == '__main__': 
    ...
    x_pub = rospy.Publisher('/kite/gimbal_joint1_position_controller/command', Float64, queue_size=1)
    y_pub = rospy.Publisher('/kite/gimbal_joint2_position_controller/command', Float64, queue_size=1)
    ...
    try:
        ...
        print msg 
        print val(x_ang, y_ang, tether_len)
        while(1): 
                key = getKey() 
